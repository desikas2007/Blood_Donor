  ...
  return (
    <div>
      <div className="mb-6">
        ...
        <p className="text-[15px] text-muted mt-1">Connect with donors and coordinate blood support.</p>
      </div>

      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        {[
          { label: "Active Requests", value: String(requests.filter((r) => r.status === "pending").length) },
          { label: "Accepted", value: String(requests.filter((r) => r.status === "accepted").length) },
          { label: "Completed", value: String(requests.filter((r) => r.status === "completed").length) },
          { label: "Registered Donors", value: String(donors.length) },
        ].map((stat, i) => (
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {donors.map((donor) => (
          <div key={donor.id} className="bg-white border border-border rounded-lg p-4">
            ...
            {donor.available && (
              <button onClick={() => { setSelectedDonor(donor); setModalOpen(true); }} className="w-full h-9 bg-red-600 text-white rounded-md text-[13px] font-medium hover:bg-red-700 transition-colors">
                Send Request
              </button>
            )}
          </div>
